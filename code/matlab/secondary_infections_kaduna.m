function secondary_infections_kaduna(filename)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
%ADJUSTABLE PARAMETERS
p = 0.90;                                                                                      %Probability of mosquito survival per feeding cycle in Kaduna, Nigeria                                   
...
max_lifespan = ceil((log(10^-2)/(-1*g)));                                                      %max_lifespan = 47
normal_mosquitodeath = 1/(0.9909);                                                             %Normalizing constant for function mosquito_death
normal_incubationprobability = 1/(normcdf(18,11.6,2.472235)-normcdf(0,11.6,2.472235));         %Normalizing constant for function incubation_probability
...
incubation_probability = @(x) normal_incubationprobability*((normcdf(x,11.6,2.472235))-normcdf(x-1,11.6,2.472235));%Anonymous function. Represents probability of incubation period on any day X. Taken from norm(5,2)
...
for i = 1:len_data
    for j = 1:max_lifespan
        ...
        for k = 0:incubation_max                                                                           %normcdf(18,11.6,2.472235) = 0.9952;
...
%filename_final = 'new_drug_secondary_probabilities_kaduna.xlsx';
...
display(size(serial_interval,1));
...
filename_final = 'new_drug_serialinterval_kaduna.csv';
