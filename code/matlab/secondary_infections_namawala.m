function secondary_infections_namawala(filename)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
%ADJUSTABLE PARAMETERS
p = 0.83;                                                                                      %Probabilty of mosquito survival per day in Namawala, Tanzania (0.83)
...
normal_mosquitodeath = 1/(0.99143);                                                             %Normalizing constant for function mosquito_death
normal_incubationprobability = 1/(normcdf(18,11.15,2.472235)-normcdf(0,11.15,2.472235));                                 %Normalizing constant for function incubation_probability
...
incubation_probability = @(x) normal_incubationprobability*((normcdf(x,11.15, 2.472235))-normcdf(x-1,11.15,2.472235));%Anonymous function. Represents probability of incubation period on any day X. Taken from norm(5,2)
...
%filename_final = 'new_drug_secondary_probabilities_namawala.xlsx';
...
serial_interval = zeros(size(generation_time,1)+abs(1 - size(bite_to_clinic,1)));
...
filename_final = 'new_drug_serialinterval_namawala.csv';
